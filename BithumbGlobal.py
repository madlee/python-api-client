...
class BithumbGlobalRestAPI:
    ...
    def query_order(self, symbol, order_id):
        page = 1
        fee = 0
        filled_v, filled_a = 0, 0

        while True:
            trans = self.order_detail(order_id, page, 10)
            for t in trans['list']:
                fee += float(t['fee'])
                filled_v += float(t['loseCount'])
                filled_a += float(t['getCount'])

            if trans['num'] <= page*10:
                break
            page += 1
        filled_a += fee

        order = self.post('singleOrder', {'orderId': order_id})
        return order
